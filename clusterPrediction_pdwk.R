...
zTrain <- fread("newtrainOutput_z.txt", header = FALSE)
zMatrixTrain <- as.matrix(zTrain)

zTest <- fread("newtestOutput_z.txt", header = FALSE)
...
finalTrainClusters <- c(train$Clusters, predictedClustersForTest)
finalTestClusters <- c(predictedClustersForTrain, test$Clusters)


print(adjustedRandIndex(finalTrainClusters, c(train$group, test$group)))

print(adjustedRandIndex( finalTestClusters, c(train$group, test$group)))

print(adjustedRandIndex(c(train$Clusters,   test$Clusters), c(train$group, test$group)))
...
finalClustering <- maxpear(myPSM)$cl  #This may take a very long time to run.

print(adjustedRandIndex(finalClustering,  c(train$group, test$group) ))

# pdf(file = "consensusMatrix.pdf", width = 10, height = 10)
